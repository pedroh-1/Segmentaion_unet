...
if args.viz:
            ...
            # 1. Preparar a Imagem Original
            img_np = np.array(img)

            # 2. A Máscara da Folha
            # A variável 'mask' já é uma matriz 2D pronta com os números das classes.
            # Vamos separar apenas onde o modelo marcou como folha (classe 1).
            leaf_mask_boolean = (mask == 1)

            # 3. Recortar a folha (A Quarta Imagem)
            # Cria a imagem preta e copia a cor original apenas onde é folha
            segmented_color_img = np.zeros_like(img_np)
            for i in range(3):
                segmented_color_img[:, :, i] = img_np[:, :, i] * leaf_mask_boolean

            # 4. Montar a tela com as 4 imagens
            ...
            ax3.set_title("Leaf Mask (Binary)")
            ax3.imshow(leaf_mask_boolean, cmap='gray')
            ax3.axis('off')
            
            ax4 = fig.add_subplot(1, 4, 4)
            ax4.set_title("Segmented Result")
            ax4.imshow(segmented_color_img)
            ax4.axis('off')
